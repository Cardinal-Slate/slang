/* tests/slang.c — Point and Vector: typing and fit are order, deriving is compose.
...
int main(void) {
  ...
  CHECK(slang_has(tagged(VECTOR), tagged(VECTOR)) != 0, "Vector{3,4} : Vector");
  CHECK(slang_has(tagged(POINT),  tagged(VECTOR)) == 0, "Point{3,4} is NOT a Vector");

  /* ③ fit is order — length : Vector → Scalar accepts a Vector, rejects a Point */
  slate_psda *length = slang_fn(spare(), tagged(VECTOR), tagged(SCALAR));
  CHECK(slang_fits(tagged(VECTOR), length) != 0, "a Vector fits length");
  CHECK(slang_fits(tagged(POINT),  length) == 0, "a Point does not fit length");

  /* ④ deriving is compose — direction : Point → Vector  ∘  length : Vector → Scalar  =  distance */
  slate_psda *direction = slang_fn(spare(), tagged(POINT),  tagged(VECTOR));
  slate_psda *length2   = slang_fn(spare(), tagged(VECTOR), tagged(SCALAR));
  slate_psda *distance  = slang_derive(spare(), direction, length2);
  CHECK(distance != 0, "direction and length fit at Vector → compose");
  CHECK(slang_has(cse_cell_payload(distance), tagged(POINT))  != 0, "distance's input is Point");
  CHECK(slang_has(cse_cell_rest(distance),    tagged(SCALAR)) != 0, "distance's output is Scalar");

  /* the wrong order does not fit: length ∘ direction (Scalar ≠ Point) */
  CHECK(slang_derive(spare(), length2, direction) == 0, "length ∘ direction does NOT fit");

  printf(fails ? "slang: FAIL\n" : "slang: ok\n");
  return fails ? 1 : 0;
}
